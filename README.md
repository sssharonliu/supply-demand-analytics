...
### 6. Deploy to Streamlit Community Cloud *(optional)*

The dashboard can be deployed publicly without exposing the database. Pre-computed CSVs are committed to the repo so Streamlit Cloud loads them directly.

**One-time setup (run locally after any data change):**

```bash
python prepare_deployment.py   # generates dashboards/*.csv
...
2. Select repository → `dashboard.py` as the main file
3. Under **App Settings → Secrets**, paste (only needed if you want live DB queries locally via Cloud):
   ```toml
   DB_USER     = "your_user"
   ...
   ```
4. Click **Deploy** — the app goes live at a public URL within ~2 minutes
